...
/*   Updated: 2020/02/28 15:03:30 by jdussert         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_check(char *map, char *title)
{
	...
	while ((n = get_next_line(fd, &line)) == 1 && line[i] != '1')
	{
		...
		else if ((line[i] == 'F' && info->f_path == -1) || (line[i] == 'C'
		&& info->c_path == -1))
			line[i] == 'F' ? ft_colors(line, &info->f_path, &i) :
			ft_colors(line, &info->c_path, &i);
	...
	if ((ft_check_existence(info, line, &i, 1) == 1) &&
	n == 1 && line[0] == '1')
